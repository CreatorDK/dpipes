...
class ClientTest11Class : public ClientTest {
public:
	ClientTest11Class(TestRegistrationClient registration) :
...
private:
	...
	static void SendRequestSync(DPClient* dlclient, ClientTest11Class* test) {
		wstringstream ss;
	...
	static void SendRequestAsync(DPClient* dlclient, ClientTest11Class* testClassPtr) {
...
public:
	void Execute(start_params_client& params) override {
		...
		_dpclient = new DPClient(_dpipe, false);
		...
		WriteClientLineW() << L"1. Connecting to server" << END_LINE;
...
TestRegistrationClient ClientTest11() {
	...
	registration.name = L"Test11";
	registration.title = L"DPServer - sync, DPClient - sync";
	registration.description = L"Description: Testing DPServer and DPClient communication";
	registration.createHandler = [registration]() { return new ClientTest11Class(registration); };
	return registration;
}