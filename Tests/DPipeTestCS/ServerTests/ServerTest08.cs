...
{
    [ServerTest("Test8", "DPipeMessanger (Unicode) server - sync, client - async", "Description: Testing scenario when DPipeMessanger server in sync mode and client in async mode", true)]
    public class ServerTest08 : ServerTestBaseClass
    {
        public ServerTest08(string testName, string title, string description) : base(testName, title, description) { }
