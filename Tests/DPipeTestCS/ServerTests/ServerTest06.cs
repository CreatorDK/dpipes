...
{
    [ServerTest("Test6", "DPipeMessanger (UTF-8) server - async, client - async", "Description: Testing scenario when DPipeMessanger server and client handling messages in async mode", true)]
    public class ServerTest06 : ServerTestBaseClass
    {
        public ServerTest06(string testName, string title, string description) : base(testName, title, description) { }
