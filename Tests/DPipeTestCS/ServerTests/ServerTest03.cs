...
{
    [ServerTest("Test3", "DPipeMessanger (UTF-8) server - sync, client - sync", "Description: Testing scenario when DPipeMessanger server and client handling messages in sync mode", true)]
    public class ServerTest03 : ServerTestBaseClass
    {
        public ServerTest03(string testName, string title, string description) : base(testName, title, description) { }
