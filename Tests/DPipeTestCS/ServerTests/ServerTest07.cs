...
{
    [ServerTest("Test7", "DPipeMessanger (Unicode) server - sync, client - sync", "Description: Testing scenario when DPipeMessanger server and client handling messages in sync mode", true)]
    public class ServerTest07 : ServerTestBaseClass
    {
        public ServerTest07(string testName, string title, string description) : base(testName, title, description) { }
