...
{
    [ClientTest("Test4", "DPipeMessanger (UTF-8) server - sync, client - async", "Description: Testing scenario when DPipeMessanger server in sync mode and client in async mode", true)]
    public class ClientTest04 : ClientTestBaseClass
    {
        public ClientTest04(string testName, string title, string description) : base(testName, title, description) { }
