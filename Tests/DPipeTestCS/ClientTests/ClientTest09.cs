...
{
    [ClientTest("Test9", "DPipeMessanger (Unicode) server - async, client - sync", "Description: Testing scenario when DPipeMessanger server in async mode and client in sync mode", true)]
    public class ClientTest09 : ClientTestBaseClass
    {
        public ClientTest09(string testName, string title, string description) : base(testName, title, description) { }
